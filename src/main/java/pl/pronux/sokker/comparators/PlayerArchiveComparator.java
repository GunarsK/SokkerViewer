...
public class PlayerArchiveComparator implements SVComparator<PlayerArchive>, Sort {
	...
	public int compare(PlayerArchive p1, PlayerArchive p2) {
		...
		switch (column) {
			case COUNTRY:
				rc = Compare.values(p1.getCountryId(), p2.getCountryId());
				break;
			case ID:
				rc = Compare.values(p1.getId(), p2.getId());
				break;
			case NAME:
			...
			case SURNAME:
				...
				break;
			case YOUTH_TEAM_ID:
				rc = Compare.values(p1.getYouthTeamId(), p2.getYouthTeamId());
				break;
			case NOTE:
