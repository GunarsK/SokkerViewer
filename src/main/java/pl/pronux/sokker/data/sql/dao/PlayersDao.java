...
public class PlayersDao {
	...
	public void addPlayerSkills(int id, PlayerSkills skills, Date date, int trainingId) throws SQLException {
		...
		ps.setInt(35, skills.getTrainingInjuryDays());
		ps.executeUpdate();
		ps.close();
	...
	public int updateTrainingAssignment(PlayerSkills skills, int trainingId) throws SQLException {
		PreparedStatement ps = connection.prepareStatement("UPDATE player_skills SET training_position = ?, training_slot = ?, pass_training = ?, training_intensity = ?, minutes_official = ?, minutes_friendly = ?, minutes_national = ?, training_injury_days = ? WHERE id_player_fk = ? AND id_training_fk = ?");
		...
		ps.setInt(8, skills.getTrainingInjuryDays());
		ps.setInt(9, skills.getPlayerId());
		ps.setInt(10, trainingId);
